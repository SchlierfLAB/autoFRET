...
@jit(nopython=True)
def collect_photons(Photons_Raw, photonHIST, bLengthLong, bStartLong, PhotonsSGR0, edges):
    ...
    for i in range(len(bStartLong)):
        ...
        Bursts[lInd:lInd+int(bLengthLong[i]),:] = np.column_stack[np.ones(int(bLengthLong[i])).T * (i+1),
        ...
        photonHIST[:,0] = photonHIST[:,0] + hAll
        hAll,_ = histc(Photons_Raw[(Photons_Raw[:,0] != 15)
                                   & ((Photons_Raw[:,0] == 2) | (Photons_Raw[:,0]==4))
                                   & ((Photons_Raw[:,2] >= PhotonsSGR0[bStartLong[i],2])
                                      &  (Photons_Raw[:,2] <= PhotonsSGR0[bStartLong[i] + int(bLengthLong[i]), 2])),1]
                       , edges)
        photonHIST[:,1] = photonHIST[:,1] + hAll

    return photonHIST, Bursts



def LOgetBurstAll(filename, pathname, suffix, lastBN, roiRG, roiR0, threIT, threIT2, minPhs, threAveT, IRF_G,
    ...
    photonHIST = dataAll.item().get('photonHIST')
    ...
    if indexSig.size != 0:
        ...
        # collect Photons
        photonHIST, Bursts = collect_photons(Photons_Raw, photonHIST, bLengthLong, bStartLong, PhotonsSGR0, edges)

        del Photons_Raw

        # channel background histogram and background counts
        np.save(strAllHIST, photonHIST)
