...
class MainWindow(QtWidgets.QMainWindow, Ui_MainWindow):
    ...
    def openFolderSlot(self):
        ...
        if len(self.fileDict) > 0:
            ...
            self.dataList = np.empty((0,23), int)
...
class MainWindow_single_well(QtWidgets.QMainWindow, Ui_MainWindow_Single_Well):
    ...
    def openFolderSlot(self):
        ...
        dialog.setNameFilter('(BData1.bin)') # should only select a BData1.bin file
        ...
        try:
            self.single_well_data = self.single_well_data.reshape((int(self.single_well_data.shape[0]/23), 23))


        except ValueError:
            with Path(filename).open('rb') as f:
                fsz = os.fstat(f.fileno()).st_size
                py_out = np.load(f, allow_pickle=True)
                while f.tell() < fsz:
                    self.single_well_data = np.vstack((py_out, np.load(f,allow_pickle=True)))
        except:
            self.settWin.setDisabled(True)
            self.msg.setIcon(self.crit_icon)
            self.msg.setText("While preprocessing an input file a error occured")
            self.msg.setInformativeText('Check if the BData are in correct shape')
            self.msg.exec()
            return

        # store data in pandas
        self.dataDF['Well'] = pd.DataFrame(self.single_well_data, columns=df_columns)
        self.updateLeftGraph()
        self.settWin.setDisabled(False)
